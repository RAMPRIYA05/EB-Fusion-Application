...
public class UserImpl implements UserDAO {
	...
	public boolean saveDetails(User user) {
		...
		String selectQuery = "Select email_id,password,phone_number,aadhaar_number from user WHERE email_id=? AND password=? AND phone_number=? AND aadhaar_number=?";
		List<String> existingUsers = jdbcTemplate.queryForList(selectQuery, String.class, user.getEmailId(),
				user.getPassword(), user.getPhoneNumber(), user.getAadhaarNumber());
		if (!existingUsers.isEmpty()) {
			return false;
		}

		String insert = "insert into user(name,email_id,password,phone_number,aadhaar_number,user_type)values(?,?,?,?,?,?)";
		Object[] params = { user.getName(), user.getEmailId(), user.getPassword(), user.getPhoneNumber(),
				user.getAadhaarNumber(), user.getUserType() };
		int rowsAffected = jdbcTemplate.update(insert, params);
